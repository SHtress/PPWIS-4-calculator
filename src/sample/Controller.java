...
public class Controller {
    ...
    private final String[] FUNCTIONS = {"pow", "sqrt", "inverse", "%"};
    ...
    public void calculate() throws ParseException {
        while (!stackRPN.empty()) {
            if(isNumber(stackRPN.peek())){
                ...
                stackTree.push(new TreeItem<>(stackAnswer.peek()));
            } else if (isFunction(stackRPN.peek())) {
                switch (stackRPN.pop()) {
                    case "sqrt": {
                        String result = sqrt(stackAnswer.pop());
                        stackAnswer.push(result);
                        TreeItem<String> sqrTree = new TreeItem<>(result+"(sqrt)");
                        sqrTree.getChildren().add(stackTree.peek());
                        stackTree.pop();
                        stackTree.push(sqrTree);
                        break;
                    }
                    case "pow": {
                        String power = stackAnswer.pop();
                        String number = stackAnswer.pop();
                        String result = pow(number, power);
                        stackAnswer.push(result);
                        TreeItem<String> powTree = new TreeItem<>(result+"(pow)");
                        powTree.getChildren().add(stackTree.peek());
                        stackTree.pop();
                        powTree.getChildren().add(stackTree.peek());
                        stackTree.pop();
                        stackTree.push(powTree);
                        break;
                    }
                    case "inverse": {
                        String result = inverse(stackAnswer.pop());
                        stackAnswer.push(result);
                        TreeItem<String> inverseTree = new TreeItem<>(result+"(inverse)");
                        inverseTree.getChildren().add(stackTree.peek());
                        stackTree.pop();
                        stackTree.push(inverseTree);
                        break;
                    }
                    case "%": {
                        String result = percent(stackAnswer.pop());
                        stackAnswer.push(result);
                        TreeItem<String> percentTree = new TreeItem<>(result+"(%)");
                        percentTree.getChildren().add(stackTree.peek());
                        stackTree.pop();
                        stackTree.push(percentTree);
                        break;
                    }
                }
                ...
                switch (stackRPN.pop()) {
                    case "+": {
                        ...
                        plusTree.getChildren().add(stackTree.peek());
                        stackTree.pop();
                        plusTree.getChildren().add(stackTree.peek());
                        stackTree.pop();
                        stackTree.push(plusTree);
                        break;
                    }
                    case "-": {
                        String secondNumber = stackAnswer.pop();
                        String firstNumber = stackAnswer.pop();
                        String result =minus(firstNumber, secondNumber);
                        stackAnswer.push(result);
                        TreeItem<String> minusTree = new TreeItem<>(result+"(-)");
                        minusTree.getChildren().add(stackTree.peek());
                        stackTree.pop();
                        minusTree.getChildren().add(stackTree.peek());
                        stackTree.pop();
                        stackTree.push(minusTree);
                        break;
                    }
                    case "*": {
                        String secondNumber = stackAnswer.pop();
                        ...
                        multiplyTree.getChildren().add(stackTree.peek());
                        stackTree.pop();
                        multiplyTree.getChildren().add(stackTree.peek());
                        stackTree.pop();
                        stackTree.push(multiplyTree);
                        break;
                    }
                    case "/": {
                        String secondNumber = stackAnswer.pop();
                        String firstNumber = stackAnswer.pop();
                        String result =divison(firstNumber, secondNumber);
                        stackAnswer.push(result);
                        TreeItem<String> divisionTree = new TreeItem<>(result+"(/)");
                        divisionTree.getChildren().add(stackTree.peek());
                        stackTree.pop();
                        divisionTree.getChildren().add(stackTree.peek());
                        stackTree.pop();
                        stackTree.push(divisionTree);
                        break;
                    }
                }
            }
    ...
    public String pow(String number, String power) {
        Double result = Math.pow(Double.parseDouble(number), Double.parseDouble(power));
        return result.toString();
    }

    public String inverse(String number) {
        Double result = 1.0 / Double.parseDouble(number);
        return result.toString();
    }

    public String percent(String number) {
        Double result = Double.parseDouble(number) / 100;
        return result.toString();
    }
    ...
    public String divison(String firstNumber, String secondNumber) {
        Double result = Double.parseDouble(firstNumber) / Double.parseDouble(secondNumber);
        return result.toString();
    }
